%-----------------------------------------------------------------------
% Job saved on 18-Mar-2013 00:07:13 by cfg_util (rev $Rev: 4972 $)
% spm SPM - SPM12b (beta)
...
clear all
 ...
 contrast = 2
    ...
    load('D:\connectome\tfMRI_social\100307\social_results\smoothed\SPM.mat')
    contrast_name = SPM.xCon(1,contrast).name
    
    contrast_folder = sprintf('D:\\connectome\\anova\\%d\\', contrast);
...
matlabbatch{1,1}.spm.stats.factorial_design.des.fd.icell(1,1).scans{subject,1} = spm_select('FPListRec',sprintf('%s\\social_results\\smoothed', subject_dirs(subject,:)),sprintf('^%s_0003\\.img$', t_or_f));
...
matlabbatch{1,1}.spm.stats.factorial_design.des.fd.icell(1,2).scans{subject,1} = spm_select('FPListRec',sprintf('%s\\social_results\\smoothed', subject_dirs(subject,:)),sprintf('^%s_0012\\.img$', t_or_f));
